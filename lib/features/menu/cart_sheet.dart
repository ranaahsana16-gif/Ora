            ...
            Container(
              ...
              child: Column(
                children: [
                  Row(
                    children: [
                      const Text('Subtotal', style: TextStyle(color: OraTheme.textSecondary)),
                      const Spacer(),
                      Text('Rs. ${total.toStringAsFixed(0)}'),
                    ],
                  ),
                  if (location?.type == 'delivery' && (location?.deliveryFee ?? 0) > 0) ...[
                    const SizedBox(height: 4),
                    Row(
                      children: [
                        const Text('Delivery Fee', style: TextStyle(color: OraTheme.textSecondary)),
                        const Spacer(),
                        Text(
                          (location?.deliveryFee ?? 0) == 0 ? 'Free' : 'Rs. ${location!.deliveryFee!.toStringAsFixed(0)}',
                          style: TextStyle(
                            color: (location?.deliveryFee ?? 0) == 0 ? OraTheme.success : null,
                            fontWeight: (location?.deliveryFee ?? 0) == 0 ? FontWeight.bold : null,
                          ),
                        ),
                      ],
                    ),
